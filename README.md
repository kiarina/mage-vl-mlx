...
The `fixtures` group pins `torch`, `torchvision`, `transformers`, and
`accelerate` to exact versions because every parity number below was measured
against that environment. Moving them is a re-measurement, not a dependency
...
`tokenizers>=0.20`, so installing this package gets you the current release —
0.23.2 produces identical prompt token ids, so the two agree in practice.
